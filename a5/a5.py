...
def calc_entropy(classifications):
    ...
    for i in range(len(classes[0])):
        ting = classes[1][i]/tot
        ans = ans - ting*(math.log2(ting))
    return ans
...
def calc_information_gain(parent_classifications, classifications_by_val, val_freqs):
    result = calc_entropy(parent_classifications)
    tot = sum(val_freqs.values())
    for i in range(len(classifications_by_val.keys())):
        pop = classifications_by_val.popitem()
        ting = len(pop[1])/tot
        result = result - ting*calc_entropy(pop[1])
    return result
